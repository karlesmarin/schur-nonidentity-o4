...
% v4 (2026-07-30): RETIRA la "tercera causa de anulacion" de rango dos de v1-v3 -- esa particion
% es la rama (b) escrita con seis partes, y la rama (b) es suficiente en todo rango por las mismas
% dos lineas; afila la dicotomia (los factores uno y dos no pueden anularse, luego el perfil de
% tamano tres no se anula nunca); anade la Proposicion 3, la torre de momentos es invertible en
% forma cerrada; y da los recuentos por mecanismo. Auditoria: PARTIV_V4_AUDIT.md.
...
anulaci\'on es precisamente su lugar de ceros.
\emph{A\~nadido en la versi\'on~4:} la reducci\'on es invertible --- tres momentos devuelven los
tres enteros en forma cerrada, de modo que no pierde nada en absoluto y no s\'olo nada de lo que
los momentos ven ---, y su signo y su tama\~no son un solo objeto: el \'indice de paridad
$n_+-n_-$ de la involuci\'on del orbifold sobre $V_\lambda$, cuyo signo es el $\zeta$ de la
identidad y cuya magnitud es el volumen de la caja. Y se retira una afirmaci\'on de las
versiones~1--3: la partici\'on de rango
dos que all\'i se ofrec\'ia como tercera causa de anulaci\'on es la segunda causa, escrita con las
seis partes que el rango dos exige. La consecuencia es una compresi\'on:
...
dos de los tres factores no pueden anularse nunca --- \S\ref{sec:zero}\\
\rowcolor{softgrey}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.94\textwidth]{fig_families_es.pdf}
...
\chi_{|z|-1},
& |E|=|O|=2,\; z=|\lambda^{(0)}|-|\lambda^{(1)}|-1,\\[3pt]
\zeta\;\chi_{\nu_1-\nu_2}\;\chi_{\nu_2-\nu_3}\;\chi_{\nu_1-\nu_3+1},
& |E|=3\ \text{o}\ |O|=3,
\end{cases}}
\end{equation}
...
el sentido del tercer factor, el rango dos \emph{(el origen del signo sali\'o de esta fila en la
v4: \eqref{eq:index})}\\
\bottomrule
\end{tabular}
\end{center}
...
de ida y vuelta.\footnote{\emph{Versi\'on~4:} ese lema se usa aqu\'i en rango uno y no se reclama
en ning\'un otro sitio. En rango uno es exacto --- $D\equiv0\iff D(1)=0$ en las $3060$ particiones
con cuatro partes $\le14$, sin violaciones --- y una evaluaci\'on de orden~2 es en general s\'olo
un filtro que nunca deja escapar un cero, de modo que nada de este art\'iculo debe leerse como
afirmarlo m\'as all\'a de un par rec\'iproco.} Karmakar~\cite{Karmakar}, Thm.~1A, enuncia precisamente ese criterio para
...
una constante por $s_{\lambda^{(0)}}(1,t,t^{-1})$ --- $0$ de los $109$ casos no triviales con
cuatro partes $\le5$ --- de modo que el objeto
con $t$ libre no es sin m\'as el car\'acter de la representaci\'on del centralizador cuya dimensi\'on
calcula el Thm.~1B de Karmakar.\footnote{\emph{Versi\'on~4:} las versiones~1--3 daban esto como
``$0$ de $105$ casos'' sin decir sobre qu\'e rango, y ning\'un rango que podamos reconstruir hoy
produce $105$; el recuento se reenuncia con el rango sobre el que se ha vuelto a correr. El
negativo en s\'i no cambia, y es algo m\'as fuerte de lo impreso: el \'unico caso proporcional en
cualquier rango probado es la partici\'on vac\'ia, donde ambos lados valen la constante~$1$.})
...
\paragraph{Por qu\'e la dicotom\'ia est\'a forzada --- y una correcci\'on a las versiones~1--3.}
Aquellas versiones dec\'ian que no hay tercera rama pero que no sab\'iamos por qu\'e no podr\'ia
haberla, y cerraban con: \emph{``la dicotom\'ia es un enunciado de rango uno, y llamarla hecho
estructural ser\'ia excederse''}. La primera mitad tiene una respuesta de dos l\'ineas que
estaba a nuestro alcance y se nos pas\'o; la segunda es falsa, y falsa en la direcci\'on que
cuesta m\'as ver --- nos quedamos cortos, y luego justificamos el quedarnos cortos con un
contraejemplo que no lo es.

\emph{Por qu\'e dos y no tres.} Porque dos de los tres factores de \eqref{eq:closedform} no
pueden anularse nunca. Un factor $\chi_k$ es cero s\'olo en $k=-1$, y los dos primeros \'indices
son $\lambda^{(0)}_1-\lambda^{(0)}_2$ y $\lambda^{(1)}_1-\lambda^{(1)}_2$, diferencias de partes
consecutivas de una partici\'on y por tanto no negativas; en el perfil de tama\~no tres los tres
\'indices son $\nu_1-\nu_2$, $\nu_2-\nu_3$ y $\nu_1-\nu_3+1$, los dos primeros no negativos y el
tercero estrictamente positivo. As\'i que el \'unico \'indice que puede degenerar es el t\'ermino
cruzado del perfil equilibrado, y la \'unica otra v\'ia hacia el cero es una clase de paridad
vac\'ia. Dos ramas, y sin sitio para una tercera. En la misma l\'inea se sigue que \textbf{el
perfil de tama\~no tres no se anula nunca} --- un enunciado que las versiones~1--3 pod\'ian haber
hecho desde su propia f\'ormula y no hicieron.

\emph{Y la dicotom\'ia no es un artefacto del rango uno.} Las versiones~1--3 ofrec\'ian
$\lambda=(3,2,2,1,1,0)$ en rango dos como partici\'on que se anula ``con ambas clases de paridad
ocupadas, que no es ninguna de nuestras dos causas''. Que las clases de paridad est\'en ocupadas
descarta la primera causa y nada m\'as. Escrita con las seis partes que el rango dos exige, esa
partici\'on cumple $\lambda_i+\lambda_{7-i}=3$ para todo $i$: es autocomplementaria de anchura
impar, que es exactamente la segunda causa. Y la segunda causa tampoco est\'a confinada al rango
uno. El argumento folcl\'orico que se registra m\'as abajo usa s\'olo que el alfabeto es estable
bajo inversi\'on y tiene determinante $-1$, y $(1,-1,z_1,z_1^{-1},\dots,z_r,z_r^{-1})$ es un
alfabeto as\'i para todo $r$; de modo que la autocomplementariedad de anchura impar fuerza la
anulaci\'on con cualquier n\'umero de pares rec\'iprocos, nuestro propio ejemplo incluido. Lo que
no podemos zanjar desde aqu\'i es el rec\'iproco --- si se anula algo \emph{m\'as} pasado el rango
uno --- y \S\ref{sec:ranktwo} lo dice.

La descomposici\'on es exacta y no deja nada. De las $560$ particiones que se anulan en el
intervalo $\text{partes}\le16$, a $420$ las mata una clase de paridad vac\'ia y a las $140$
restantes un \'indice cruzado degenerado, y a ninguna ninguna otra cosa
(Figura~\ref{fig:zerolocus}). Ese reparto es por \emph{mecanismo}, y es disjunto; las dos
abreviaturas en $\lambda$ del recuadro de arriba no lo son, y reparten las mismas $560$ como $420$
y $240$, con $100$ particiones que cumplen ambas.

\begin{figure}[h]
\centering
\includegraphics[width=0.86\textwidth]{fig_zerolocus_es.pdf}
\caption{El lugar de ceros sobre el rango $\text{partes}\le16$ --- el mismo rango del
$560=420+140$ de arriba, y se dice aqu\'i porque una l\'amina dibujada sobre otro rango es otra
l\'amina ---, por tama\~no y primera parte. Las celdas azules contienen
una partici\'on matada por $\beta$-paridad alternante, las naranjas una matada por
autocomplementariedad con constante impar, las partidas ambas; hay $156$ celdas ocupadas. El
contenido de la l\'amina es
...
entonces $s_\lambda(p)=(-1)^cs_\lambda(p)$, y $c$ impar fuerza cero. Nada en esas dos l\'ineas usa
cuatro letras: con $N=2r+2$ de ellas y $\lambda^\star_i=c-\lambda_{N+1-i}$ el c\'alculo es
id\'entico, porque lo \'unico que consume es que el alfabeto sea estable bajo inversi\'on y tenga
determinante $-1$ --- que es en lo que se apoyan \S\ref{sec:zero} y \S\ref{sec:ranktwo} al decir
que la rama sobrevive a cualquier n\'umero de pares rec\'iprocos. Esta es la sombra, en
...
$D=\zeta\,\chi_p\chi_q\chi_r$\footnote{\emph{Corregido en la versi\'on~4.} En toda esta secci\'on
$\zeta$ denota el signo \emph{total} de \eqref{eq:closedform}, que en el perfil equilibrado es el
$\zeta$ de \eqref{eq:sign} por $\mathrm{sgn}(z)$ y en el de tama\~no tres es \eqref{eq:sign} sin
m\'as. Las versiones~1--3 usaban el mismo s\'imbolo para las dos cosas, y difieren en $700$ de las
$3060$ particiones con cuatro partes $\le14$ --- exactamente las equilibradas con $z<0$ ---, de
modo que quien sustituyera \eqref{eq:sign} en \eqref{eq:moments} obtendr\'ia all\'i el signo
cambiado en todos los momentos. Nada m\'as cambia en la secci\'on: todos los cocientes de abajo
dividen $\zeta$ fuera, y el \'unico sitio donde se usa el $M_0$ con signo por s\'i solo,
\eqref{eq:index}, es justo donde ese signo resulta ser lo importante.} entonces $\delta$ es la \emph{convoluci\'on triple} de tres
...
\paragraph{Qu\'e es $\zeta$, y qu\'e pesa la caja (respondido en la versi\'on~4).} Las
versiones~1--3 preguntaban qu\'e significa el signo y contestaban que un signo de ordenaci\'on de
un beta-conjunto que decide el signo de una curvatura f\'isica es ``o una coincidencia de
contabilidad o la sombra de algo, y no sabemos distinguirlo''. Es lo segundo, y la sombra ocupa
una l\'inea. El momento cero es $M_0(\delta)=\sum_m\delta(m)=D(1)$, y $D(1)=s_\lambda(1,-1,1,1)$
es la traza del elemento de orden dos $\sigma=\mathrm{diag}(1,-1,1,1)$ sobre $V_\lambda$ ---
que es, salvo conjugaci\'on, la matriz de paridad $P_0=\mathrm{diag}(+,+,+,-)$ de la Parte~III.
Escribiendo $n_\pm$ para las dimensiones de sus dos autoespacios,
\begin{equation}\label{eq:index}
\boxed{\;M_0(\delta)\;=\;n_+-n_-\;,\qquad
(p{+}1)(q{+}1)(r{+}1)\;=\;\bigl|\,n_+-n_-\,\bigr|\;}
\end{equation}
de modo que $\zeta$ es el \emph{signo del \'indice de paridad de la involuci\'on del orbifold}, y
el volumen de la caja es su magnitud (Figura~\ref{fig:index}). Ambas mitades se comprobaron en las $3060$ particiones con
cuatro partes $\le14$, con el \'indice recalculado por una v\'ia independiente --- contando los
pesos de $V_\lambda$ por su paridad bajo $\sigma$ y no a trav\'es de la forma cerrada ---: sin
fallos.

Y la \emph{ordenaci\'on} tampoco es casualidad. El numerador de Weyl es el bialternante; separar
el beta-conjunto por clase de residuo es separar ese determinante en los bloques donde $\sigma$
act\'ua por $+1$ y por $-1$; e $\mathrm{inv}$ cuenta exactamente las inversiones de esa
separaci\'on. Aparece un signo de ordenaci\'on porque ordenar es lo que hace una
diagonalizaci\'on por bloques, y el signo que lleva es el de la permutaci\'on que la ejecuta.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_index_es.pdf}
\caption{El criterio de anulaci\'on, le\'ido como enunciado sobre la proyecci\'on del orbifold.
\textbf{Izquierda:} todas las particiones con cuatro partes $\le12$, por tama\~no frente a su
\'indice de paridad $n_+-n_-$; la escala es logar\'itmica sim\'etrica, de modo que el \'indice $0$
es una recta y no un l\'imite. El car\'acter se anula id\'enticamente \emph{exactamente} sobre esa
recta, y el contenido de la l\'amina es qu\'e particiones la alcanzan: las dos ramas de
\eqref{eq:zerolocus}, con el tama\~no del marcador dado por cu\'antas particiones de ese tama\~no
aporta cada una, y ning\'un tercer color en ninguna parte. \textbf{Derecha:} el mismo \'indice
dibujado como lo que \eqref{eq:index} dice que es. Los tres enteros del $2$-cociente son las
aristas de una caja cuyo volumen es $|n_+-n_-|$; una representaci\'on de dimensi\'on $10{,}920$
puede fallar el equilibrio por $12$. Las dos del medio difieren por el twist del determinante
$\lambda\mapsto\lambda+(1^4)$: misma caja, signo opuesto, misma dimensi\'on --- por eso la caja
fija la magnitud del \'indice y no su signo. El color lleva la rama a la izquierda y nada a la
derecha; el signo va escrito.}
\label{fig:index}
\end{figure}

La lectura f\'isica se sigue sin m\'as trabajo. $n_+=\tfrac12(\dim V_\lambda+M_0)$ es el n\'umero
de componentes de $V_\lambda$ pares bajo la paridad del orbifold, $n_-$ el de las impares, y
$\zeta$ registra por tanto \emph{cu\'al de las dos paridades est\'a m\'as poblada}. Que la
direcci\'on de la contribuci\'on fermi\'onica a la curvatura de la l\'inea de Wilson siga a ese
n\'umero no es una coincidencia de contabilidad: es la \'unica cantidad que una proyecci\'on de
orbifold calcula. Lo que queda genuinamente abierto es la otra mitad de \eqref{eq:index}: el
volumen de la caja es un \'indice, luego los tres enteros no son s\'olo una compresi\'on --- su
producto es un invariante de teor\'ia de representaciones, y no sabemos de qu\'e son invariantes
...
\paragraph{Y la torre se puede recorrer hacia atr\'as (a\~nadido en la versi\'on~4).} La
Proposici\'on~\ref{prop:ring} dice que los momentos son funciones de los tres \'indices. El
rec\'iproco tambi\'en es cierto, y en una forma m\'as fuerte de lo que la pregunta suele admitir:
bastan \emph{tres} momentos, y la inversi\'on es en forma cerrada.

\begin{proposicion}[la compresi\'on no pierde nada y es invertible]\label{prop:inverse}
Sea $\delta$ como arriba y sean $e_1,e_2,e_3$ las funciones sim\'etricas elementales de
$C_p,C_q,C_r$. Entonces
\begin{equation}\label{eq:inverse}
\boxed{\;
e_1=\frac{3M_2}{M_0},\qquad
e_2=\frac{15}{4}\,\frac{M_4}{M_0}-\frac34 e_1^{\,2}+e_1,\qquad
e_3=M_0^{\,2}-1-e_1-e_2,\;}
\end{equation}
y $\{C_p,C_q,C_r\}$ es el multiconjunto de ra\'ices de $x^3-e_1x^2+e_2x-e_3$, de modo que
$\{p,q,r\}=\{\sqrt{C+1}-1\}$ se recupera s\'olo de $M_0$, $M_2$ y $M_4$.
\end{proposicion}

\begin{proof}
La primera identidad es \eqref{eq:casimir}, y la segunda es \eqref{eq:casimir} otra vez, resuelta
para $e_2$ tras sustituir $C_p^2+C_q^2+C_r^2=e_1^2-2e_2$. Para la tercera,
$M_0=\pm(p{+}1)(q{+}1)(r{+}1)$ por \eqref{eq:moments} y $(k+1)^2=k(k+2)+1=C_k+1$, luego el signo
desaparece en $M_0^{\,2}=\prod_k(C_k+1)=1+e_1+e_2+e_3$. Un multiconjunto de tres n\'umeros son sus
funciones sim\'etricas elementales, y $k\mapsto C_k$ es inyectiva sobre los enteros no negativos.
\end{proof}

\noindent Comprobado en las $2925$ ternas con $p,q,r\le24$: la inversi\'on devuelve el
multiconjunto correcto en todos los casos, sin colisiones. Conviene separar dos cosas. La
identidad $M_0^2=\prod(C_k+1)$ es lo que hace que basten tres momentos y no cuatro --- $M_0$ no es
una mera normalizaci\'on, lleva $e_3$ dentro --- y es el mismo $(k+1)^2=C_k+1$ que mueve la
demostraci\'on de la Proposici\'on~\ref{prop:ring}. Y el enunciado tiene lectura del lado
f\'isico de la Parte~III: la reducci\'on $\Phi=F\circ Q$ no s\'olo no pierde nada de lo que los
momentos ven, no pierde nada en absoluto. Los tres enteros y la sucesi\'on observable de momentos
son dos sistemas de coordenadas para el mismo objeto, y cada uno se calcula del otro en forma
cerrada. Condicionada a la Observaci\'on~\ref{obs:main}, por supuesto: la inversi\'on hereda su
estatus, como todo en esta secci\'on.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{fig_spectrum3d_es.pdf}
...
significa $\zeta$?} Esa pregunta estaba abierta en las versiones~1--3 y queda contestada m\'as
arriba, en \eqref{eq:index}: es el signo del \'indice de paridad $n_+-n_-$ de la involuci\'on del
orbifold, y el volumen de la caja es ese \'indice en valor absoluto. Lo que queda de la pregunta
es la mitad m\'as fina: los tres factores por separado.
...
\paragraph{Qu\'e es en realidad la \'ultima fila (corregido en la versi\'on~4).} Las
versiones~1--3 la le\'ian como lo m\'as afilado que pod\'ia decirse sobre el rango dos: una
anulaci\'on ``con ambas clases de paridad ocupadas'', luego una regla \emph{estrictamente m\'as
fina} que la de rango uno, gobernada por algo que ``no se ve desde este lado''. Que las clases de
paridad est\'en ocupadas excluye la primera de nuestras dos ramas y no dice nada de la segunda.
Escrita con seis partes, $(3,2,2,1,1,0)$ cumple $\lambda_i+\lambda_{7-i}=3$ para todo $i$: es
autocomplementaria de anchura impar, y por el argumento de \S\ref{sec:zero} --- que s\'olo
necesita un alfabeto estable bajo inversi\'on y de determinante $-1$, luego vale para todo $r$ ---
eso fuerza la anulaci\'on. La fila no es una tercera causa. Es la segunda, en el rango donde este
art\'iculo hab\'ia dejado de buscarla, y el enunciado honesto sobre el rango dos queda por tanto
m\'as estrecho y m\'as limpio: sabemos que nuestras dos ramas siguen \emph{forzando} all\'i la
anulaci\'on, y no sabemos si algo m\'as lo hace.
...
recombinaci\'on y no una factorizaci\'on, y el cero en $(3,2,2,1,1,0)$ es una cancelaci\'on
\emph{entre} t\'erminos y no la anulaci\'on de un factor --- coherente con que sea la rama (b),
que es un enunciado sobre $\lambda$ y no sobre ning\'un t\'ermino concreto. No hemos derivado el conjunto de \'indices $\eta$, y
...
disponible sobre cualquier demostraci\'on propuesta. \emph{La versi\'on~4 a\~nade qu\'e
\emph{es} el signo}: por \eqref{eq:index} es el signo del \'indice de paridad $n_+-n_-$ de la
involuci\'on del orbifold, de modo que una demostraci\'on que produzca otro convenio de signo no
es meramente incompatible con la nuestra --- discrepa de una traza.

% ===================================================================
\section{Libro de cuentas}\label{sec:ledger}

% [!htbp], y no [h]: con el material de la versi\'on 4 la cola de flotantes se alarg\'o lo
% bastante como para dejar esta tabla detr\'as de la bibliograf\'ia.
\begin{table}[!htbp]
\centering\small
...
\midrule
\rowcolor{softgrey}
la inversi\'on \eqref{eq:inverse} \textbf{(v4)} & sus dos ingredientes son \eqref{eq:casimir} y
$(k+1)^2=C_k+1$ & nuestra, y elemental: que $M_0$ lleve $e_3$ es lo que hace que basten tres
momentos\\
\midrule
la lectura como \'indice \eqref{eq:index} \textbf{(v4)} & la traza de una involuci\'on sobre
$V_\lambda$ es objeto de manual & nuestra: la identificaci\'on de $\zeta$ con su signo y del
volumen de la caja con su magnitud\\
\bottomrule
\end{tabular}
...
ser\'a m\'as complicado que una factorizaci\'on, pues all\'i el resultado gen\'erico es una suma
con signos. \emph{La versi\'on~4 retira la segunda pieza de evidencia que dimos aqu\'i}, que en
rango dos una partici\'on se anula por una raz\'on que nuestro criterio no ve; no es as\'i, y la
correcci\'on deja la direcci\'on en mejor estado del que la dejamos. Bautizar un fen\'omeno sobre
la base de un solo rango y una identidad no demostrada seguir\'ia siendo prematuro, y seguimos
declinando hacerlo.
...
La versi\'on~4 cambia tres cosas y ninguna de ellas es la identidad. La dicotom\'ia de
\S\ref{sec:zero} est\'a forzada y no s\'olo observada --- dos de los tres factores no pueden
anularse, de modo que el perfil de tama\~no tres no se anula nunca. La reducci\'on $\Phi=F\circ Q$
es invertible en forma cerrada (Proposici\'on~\ref{prop:inverse}), lo que sube ``la f\'isica
s\'olo ve tres enteros'' a ``los tres
enteros y lo que la f\'isica ve son el mismo dato''; y el signo y el tama\~no de esa reducci\'on
resultan ser un solo objeto, el \'indice de paridad de la involuci\'on del orbifold, de modo que
\eqref{eq:index} contesta una pregunta que las versiones~1--3 plantearon y no supieron zanjar.
Y la partici\'on de rango dos que las
versiones~1--3 ofrec\'ian como tercera causa de anulaci\'on queda retirada: es la segunda causa
escrita con seis partes, y la segunda causa vale en todo rango por las mismas dos l\'ineas que la
dan aqu\'i. Lo abierto en rango dos es ya s\'olo el rec\'iproco.
